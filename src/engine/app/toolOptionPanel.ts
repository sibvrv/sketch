pg.toolOptionPanel = function () {
  ...
  const setup = function (options, components, changeCallback) {
    const $panel = jQuery('<div class="toolOptionPanel">');
    const $title = jQuery('<h3 class="panelTitle">' + options.name + '</h3>');
    ...
    jQuery.each(components, function (key, comp) {
      ...
      if (comp.type == 'boolean') {
        ...
        jQuery.each(comp.options, function (index, value) {
          const $opt = jQuery('<option value="' + value + '">' + value + '</option>');
    ...
    const $resetButton = $('<button class="toolOptionResetButton" title="Reset Tool Settings">R</button>').click(function () {
