...
pg.tools.select = function () {
  ...
  let boundsPath: any;
  const boundsScaleHandles: any[] = [];
  const boundsRotHandles: any[] = [];
  ...
  const {Tool, Shape, Point, Size} = paper;
  ...
  const activateTool = function () {
    ...
    tool.onMouseDown = function (event: any) {
      ...
      const evtPoint = event.point;

      const hitResult = paper.project.hitTest(evtPoint, hitOptions);
      ...
      if (hitResult) {
        ...
        if (hitResult.item.data && hitResult.item.data.isScaleHandle) {
          ...
          jQuery.each(rotItems, function (i: number, item: any) {
    ...
    tool.onMouseDrag = function (event: any) {
      ...
      if (mode === 'rectSelection') {
        ...
        jQuery.each(boundsScaleHandles, function (index: number, handle: any) {
          handle.position = itemGroup.bounds[getRectCornerNameByIndex(index)];
          handle.bringToFront();
        });

        jQuery.each(boundsRotHandles, function (index: number, handle: any) {
          if (handle) {
    ...
    tool.onMouseUp = function (event: any) {
      ...
      if (mode === 'rectSelection' && selectionRect) {
        ...
        jQuery.each(selectedItems, function (index: number, item: any) {
  ...
  const setSelectionBounds = function () {
    ...
    let rect: any;
    jQuery.each(items, function (index: number, item: any) {
      if (rect) {
    ...
    jQuery.each(boundsPath.segments, function (index: number, segment: any) {
      let size = 4;
      ...
      if (index === 7) {
        const offset = new Point(0, 10 / paper.view.zoom);
  ...
  const preProcessSelection = function () {
  ...
  const getRectCornerNameByIndex = function (index: number) {
    switch (index) {
      case 0:
        return 'bottomLeft';
  ...
  const getOpposingRectCornerNameByIndex = function (index: number) {
    switch (index) {
      case 0:
        return 'topRight';
